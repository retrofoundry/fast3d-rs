...
";
    ...
    // G_LOADTLUT must have expanded the packed RDRAM palette into exactly palette_count * 8 bytes
    // (stride-8 TMEM model: entry i at tlut[i<<3], 6 pad bytes per slot).
    // RDRAM itself is PACKED (count*2 bytes); load_tlut DMA-expands to this stride-8 Vec.
    // decode_ci8 reads entry i at tlut[i<<3], so all entries must be in the expanded buffer.
    // Before the encode.rs fix, lrt landed in bits[23:12] but HLE reads bits[11:0],
    // so count was always 1 → tlut.len() == 8.  After the fix, count == palette_count.
    assert_eq!(
        result.rdp.tlut.len(),
        palette_count * 8,
        "TLUT byte count wrong: got {} expected {}; was the encode.rs lrt bit-position fix applied?",
        result.rdp.tlut.len(),
        palette_count * 8
    );
    ...
    assert_eq!(
        result.rdp.tlut[0], 0x00,
        "entry 0 hi should be 0x00 (black)"
    );
    assert_eq!(
        result.rdp.tlut[1], 0x01,
        "entry 0 lo should be 0x01 (alpha=1)"
    );
    // Entry 1 = red [255,0,0,255] → RGBA16 r5=31,g5=0,b5=0,a1=1 → 0xF801.
    assert_eq!(result.rdp.tlut[8], 0xF8, "entry 1 hi should be 0xF8 (red)");
    assert_eq!(
        result.rdp.tlut[9], 0x01,
        "entry 1 lo should be 0x01 (alpha=1)"
    );
    // Entry 2 = green [0,255,0,255] → RGBA16 r5=0,g5=31,b5=0,a1=1 → 0x07C1.
    assert_eq!(
        result.rdp.tlut[16], 0x07,
        "entry 2 hi should be 0x07 (green)"
    );
    assert_eq!(
        result.rdp.tlut[17], 0xC1,
        "entry 2 lo should be 0xC1 (green+alpha)"
    );
}
