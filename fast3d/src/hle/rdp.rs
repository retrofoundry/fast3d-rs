...
use crate::hle::consts::rdp::{
    G_LOADBLOCK, G_LOADTLUT, G_NOOP, G_RDPFULLSYNC, G_RDPHALF_1, G_RDPHALF_2, G_RDPLOADSYNC,
    G_RDPPIPESYNC, G_RDPSETOTHERMODE, G_RDPTILESYNC, G_SETBLENDCOLOR, G_SETCIMG, G_SETCOMBINE,
    G_SETENVCOLOR, G_SETFILLCOLOR, G_SETFOGCOLOR, G_SETPRIMCOLOR, G_SETSCISSOR, G_SETTILE,
    G_SETTILESIZE, G_SETZIMG,
};
...
#[derive(Clone, Debug, Default, PartialEq)]
pub struct Rdp {
    pub tmem: Vec<u8>, // linear source bytes from LoadBlock (decoded later)
    ...
    /// Palette (TLUT) bytes loaded by G_LOADTLUT — models TMEM 0x800–0xFFF as a 0-based 2KB buffer.
    pub tlut: Vec<u8>,
...
fn load_block<M: Rdram>(c: &Cmd, cx: &mut Ctx<M>) {
    ...
    cx.rdp.tmem = cx.mem.read_bytes(addr, bytes).into_owned();
    cx.rsp.material_dirty = true;
}

fn load_tlut<M: Rdram>(c: &Cmd, cx: &mut Ctx<M>) {
    // LoadTLUT: count = (lrt>>2)+1 packed 16-bit big-endian entries from tex_image.addr.
    // RDRAM palette layout (hardware-accurate): entries are PACKED — count*2 bytes, contiguous.
    // Real N64 / sm64 DLs store palettes this way; gsDPLoadTLUT DMA copies packed RDRAM → strided
    // TMEM (one 16-bit entry per 8-byte TMEM slot). We model TMEM as a stride-8 Vec<u8> so that
    // decode_ci8's `tlut[index<<3]` lookup works directly. On load we DMA-expand: entry i's 2
    // packed bytes go to tlut[i*8..i*8+2], the remaining 6 bytes per slot are zeroed.
    ...
    // Expand packed RDRAM → stride-8 TMEM model (count*8 bytes): entry i at tlut[i*8..i*8+2].
    let mut tlut = vec![0u8; count as usize * 8];
    for i in 0..count as usize {
        tlut[i * 8] = packed[i * 2];
        tlut[i * 8 + 1] = packed[i * 2 + 1];
    }
    cx.rdp.tlut = tlut;
    cx.rsp.material_dirty = true;
}
...
mod tests {
    ...
    fn load_tlut_stores_be_entries() {
        ...
        // load_tlut expands packed RDRAM → stride-8 TMEM model: tlut.len() == 4*8 == 32.
        ...
        assert!(diags.is_empty());
        // Expanded TMEM model: stride-8, so tlut.len() == count * 8 == 32.
        assert_eq!(rdp.tlut.len(), 32);
        // Entry 0 at tlut[0..2]; pad bytes [2..8] are 0.
        assert_eq!(&rdp.tlut[0..2], &[0xAA, 0xBB], "entry 0 bytes");
        assert_eq!(&rdp.tlut[2..8], &[0u8; 6], "entry 0 pad");
        // Entry 1 at tlut[8..10]; pad bytes [10..16] are 0.
        assert_eq!(&rdp.tlut[8..10], &[0xCC, 0xDD], "entry 1 bytes");
        assert_eq!(&rdp.tlut[10..16], &[0u8; 6], "entry 1 pad");
        // Entry 2 at tlut[16..18].
        assert_eq!(&rdp.tlut[16..18], &[0xEE, 0xFF], "entry 2 bytes");
        // Entry 3 at tlut[24..26].
        assert_eq!(&rdp.tlut[24..26], &[0x11, 0x22], "entry 3 bytes");
    }
    ...
    fn load_tlut_packed_rdram_sm64_shape_decodes_ci8_correctly() {
        ...
        let (rdp, diags) = run_cmd(&rdram, rdp, w0, w1);
        assert!(diags.is_empty());
        // load_tlut DMA-expands packed bytes → stride-8 TMEM model.
        assert_eq!(rdp.tlut.len(), 3 * 8, "expanded TMEM must be count*8 bytes");
        // Entry bytes land at stride-8 slots.
        assert_eq!(&rdp.tlut[0..2], &[0x00, 0x01], "entry 0 = black RGBA16");
        assert_eq!(&rdp.tlut[8..10], &[0xF8, 0x01], "entry 1 = red RGBA16");
        assert_eq!(&rdp.tlut[16..18], &[0x07, 0xC1], "entry 2 = green RGBA16");
        // decode_ci8 via the expanded stride-8 TMEM.
        let ci8_src = [0u8, 1, 2];
        let out = crate::hle::texdec::decode_ci8(&ci8_src, 3, 1, &rdp.tlut, 2 /* RGBA16 */);
