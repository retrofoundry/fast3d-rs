...
/// 96 bytes total; must be ≤ 256 (A8b slot stride). Field order matches `skeleton.wgsl struct Combiner`
/// (std140): 8 scalar u32/f32 fields (32 bytes), then four vec4<f32> fields (64 bytes).
...
const _: () = assert!(std::mem::size_of::<CombinerUniform>() == 112);
...
impl TexturedPipeline {
    pub fn new(
        ...
        // group0_bgl: texture + sampler (@group(0))
...
impl SceneRenderer {
    ...
    pub fn new(
        ...
        // 1×1 (zero-filled) texture + `@group(0)` bind group used as the FillRect texture binding.
        // The pipeline layout requires group 0, but the fill combine has `tex_enable = 0`, so this
        // texture is never sampled — its contents are irrelevant (left at the default zero).
        let fill_bind_group = {
