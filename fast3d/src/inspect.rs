...
pub enum WalkTermination {
    #[default]
    End,
    /// Out-of-bounds or overflowing memory access, including legacy DlPastRdram faults.
    Bounds,
    /// A memory read failed because its data or format was unavailable.
    MemoryRead,
    /// The interpreter rejected unsupported or invalid command state and discarded the task.
    Rejected,
    Runaway,
    ObserverStopped,
    Cap,
...
pub struct WalkSummary {
    /// Command count, including a final failed fetch. A failed fetch has no callback.
    pub dispatched: u32,
