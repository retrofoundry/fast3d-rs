...
impl Renderer {
    ...
    pub fn shutdown(mut self) {
        // P4: fire the render hook's `deinit` before the device drops (RT64 fidelity). Takes the hook
...
impl Renderer {
    ...
    /// VI snapshot via `hw.vi()` itself (RT64 fidelity). Auto-reconfigures the surface from the
