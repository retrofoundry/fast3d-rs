...
fn capture_high_addresses_resolve_on_this_platform() {
    ...
    let entry = mem.resolve_masked(0x0700_0000);
    assert_eq!(entry, 0x123456000);
    assert_eq!(mem.command_stride(), 16);
    assert_eq!(mem.read_command(entry).w0, 0xed000000);
    let cimg = mem.read_command(entry + mem.command_stride());
    assert_eq!(cimg.w0, 0xff10003f);
    assert_eq!(cimg.w1, 0x34567000);
    assert_eq!(cimg.w1_addr, 0x234567000);
    assert_eq!(mem.read_u16(entry + 16), 0x003f);
    assert_eq!(&*mem.read_bytes(entry + 17, 3), &[0, 0x10, 0xff]);
    replay.check().unwrap();
