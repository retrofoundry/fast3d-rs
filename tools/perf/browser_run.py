...
def main():
    ...
    def request(method,path,value=None,timeout=3600):
    ...
    with (args.out/'browser-processes.log').open('w') as logs:
        profile=(args.out/'chrome-profile').resolve()
        profile.mkdir(exist_ok=False)
        tree=BrowserProcesses(args.out,profile)
        session=None
        try:
            tree.start(serve,stdout=logs,stderr=logs)
            tree.start([str(args.chromedriver),'--port='+str(args.driver_port)],stdout=logs,stderr=logs)
            ...
            chrome_options=options.setdefault('goog:chromeOptions',{})
            chrome_args=chrome_options.setdefault('args',[])
            if any(value.startswith('--user-data-dir') for value in chrome_args) or chrome_options.get('debuggerAddress'):
                raise ValueError('browser_run requires its own Chrome profile and session')
            chrome_args.append('--user-data-dir='+str(profile))
    ...
    '''
            ...
            tree.close((lambda: request('DELETE','/session/'+session,timeout=5)) if session else None)
