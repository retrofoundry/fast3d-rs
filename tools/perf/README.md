...
Live request traces contain owned encoded banks and resolved compatibility
streams. They leave RGBA output and decoder read-footprint fields empty. The
trace collector explicitly runs `Request::analyze` on its offline copies before
cache simulation and case collection. This developer analysis calls the CPU
format oracle directly; it does not change production rendering or add an
