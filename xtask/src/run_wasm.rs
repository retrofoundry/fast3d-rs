...
pub(crate) fn run_wasm(shell: Shell, mut args: Arguments) -> Result<(), anyhow::Error> {
    ...
    xshell::cmd!(
        shell,
        "cargo build --target wasm32-unknown-unknown --bin fast3d-examples {release_flag...}"
    )
        .args(&cargo_args)
        .quiet()
        .run()
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{output_dir}/fast3d-examples.wasm --target web --no-typescript --out-dir target/generated --out-name webgpu"
    )
        .quiet()
        .run()
    ...
    xshell::cmd!(
        shell,
        "cargo build --target wasm32-unknown-unknown --bin fast3d-examples --no-default-features --features f3dex2,webgl {release_flag...}"
    )
        .args(&cargo_args)
        .quiet()
        .run()
    ...
    xshell::cmd!(
            shell,
            "wasm-bindgen target/wasm32-unknown-unknown/{output_dir}/fast3d-examples.wasm --target web --no-typescript --out-dir target/generated --out-name webgl2"
        )
        .quiet()
        .run()
        .context("Failed to run wasm-bindgen")?;
